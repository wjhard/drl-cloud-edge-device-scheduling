...
```text
duration(i,r) = w_i / p_r
```

若前驱 `i` 与后继 `j` 分配到不同资源，则通信时延由数据量与有效带宽决定；同资源通信时延为零。任务 `j` 的开始时间必须满足：

```text
s_j >= max_{i in pred(j)} (f_i + communication(i,j,x_i,x_j))
```

同一资源上的执行区间不得重叠。目标函数为：

```text
minimize C_max, where C_max = max_i f_i
```
...
```text
ratio_k = makespan_method(k) / makespan_HEFT(k)
mean_ratio = average_k(ratio_k)
```
...
```text
non-terminal: reward = -0.01
terminal:     reward = -0.01 - (current_makespan / HEFT_makespan - 1)
```

该设计把不同任务规模的终局信号统一到相对比值尺度；`tests/test_reward_shaping.py` 通过重放 HEFT 调度验证终局奖励接近 `-0.01`。默认 `reward_mode=raw` 保留旧行为，保证历史实验可复现。
...
```text
logit(task) = rank_scale * normalized_HEFT_rank(task)
              + delta_scale * tanh(MLP(task,state))
```

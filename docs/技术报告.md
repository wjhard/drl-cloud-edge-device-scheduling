...
`env/observation_normalizer.py` 的 `ObservationNormalizer` 只标准化连续特征，不改动 `task_valid_mask`、`task_adjacency` 和 `ready_task_node_ids` 等掩码/索引字段。统计量保存在 `env/normalization_stats.json`；500 个 DAG、8038 个决策样本的原始数据集元信息见 `artifacts/diagnostic_logs/bc_dataset_sanity_check_output.log`。
...
固定步数惩罚与不同规模 DAG 的绝对 makespan 不在同一量纲，且主要性能信号只在 episode 末端出现。`diagnostics/action_distribution_check.py` 的历史输出 `artifacts/diagnostic_logs/diagnostics_action_distribution_output.log` 显示，10 个诊断 DAG 的 180 个任务全部分配给 `cloud_0`，其余三类资源为 0。该策略等价于“始终选择算力最强资源”，忽略了通信、并行执行和时间线空隙。
...
需要指出，现存 `evaluation/results/summary_reward_shaped.json` 的实际值仍为 `1.367762`，`artifacts/diagnostic_logs/reward_shaped_action_distribution_output.log` 仍记录 `cloud_0=100%`，没有支持开发背景中“1.325、cloud_0=96.1%”的可审计文件。因此，本报告不把该历史口径作为已验证结论。奖励重构解决了目标尺度问题，但从现存实验看并未单独解决策略塌缩。
...
`artifacts/diagnostic_logs/bc_batch_mask_and_normalization_check_output.log` 对 8038 个样本进行了字段审计：任务计算量最大约 9.998，后继计算量之和最大约 82.285，而资源带宽最大为 1000；扁平观测整体标准差约 30.936。如此悬殊的量纲会使网络优化偏向大数值字段。

诊断同时排除了 batch mask 交叉污染：合法动作数分别为 4、20、44 的三个样本，批量和单样本 `log_prob` 差异均为 0。93 样本过拟合实验中，未归一化 best accuracy 仅 27.957%，加入标准化后达到 90.323%。完整 joint BC 数据集以 80 epochs、`lr=0.001` 训练时最高准确率达到 99.104%，来源为 `artifacts/diagnostic_logs/mlp_bc_normalized_strength_test_80e_lr1e3_output.log`。

正式加入 `ObservationNormalizer` 后，joint MLP 的 `mean_ratio` 降至 **1.293190**，相比初版下降 0.074572，是动作空间重构前最明显的单项改进。诊断资源分布中 `cloud_0` 占比由 100% 降至 **91.11%**，而不是开发背景中的 96.1%。数据来源为 `evaluation/results/summary_mlp_normalized.json` 和 `artifacts/diagnostic_logs/mlp_normalized_action_distribution_output.log`。

## 3.5 阶段五：动作空间重构

`diagnostics/action_space_complexity_check.py` 对 20 个验证场景、350 个决策步骤统计得到：joint 环境平均每步有 12.377 个合法联合动作，平均 3.094 个候选任务，每个任务固定有 4 个资源候选，最大合法动作数为 32。更关键的是，在每个场景进行 20 次随机 rollout 后，350 个“场景—任务”对中有 310 个曾选择超过一种资源，不稳定比例为 **88.5714%**，平均每个任务出现 2.491 种资源选择。证据见 `artifacts/diagnostic_logs/action_space_complexity_check_output.log`。
...
归一化前 BC 完整数据准确率只有约 16.7%～17.2%，随机合法动作期望准确率约 7.54%；小样本过拟合最高仅 27.957%。梯度、优化器参数和 action log-prob 均经诊断确认正常，最终定位到观测量纲。归一化后，93 样本最高 90.323%；ranked 专属小数据集在 200 epochs 达到 100%，完整 ranked 数据集在 80 epochs 达到 **98.967%**。证据见 `artifacts/diagnostic_logs/bc_dataset_sanity_check_output.log`、`artifacts/diagnostic_logs/bc_batch_mask_and_normalization_check_output.log`、`artifacts/diagnostic_logs/scan_step1_ranked_bc_small_overfit.log` 和 `artifacts/diagnostic_logs/scan_step1_ranked_bc_full_pretrain_only.log`。
...
在全新克隆且无 checkpoint、无验证场景的 Windows 流水线验证中，200k 配置实际完成 200704 个 PPO rollout 步，训练耗时 **489.71 秒**，随后自动生成 20 个场景并完成评测。完整日志为 `evaluation/results/run_final_pipeline_fresh_clone_full_output.log`。开发过程曾另行统计 Monitor 累计环境步数，但对应原始 Monitor 文件未纳入最终证据归档，因此本报告不再引用该数字。

5 次 Best-of-64 独立评测平均耗时为 **27.762 ± 1.178 秒/20 场景**。作为对照，GAT joint 的 200k 历史训练耗时为 1941.22 秒，而 MLP ranked 为 413.70 秒，说明 GAT 的图注意力计算显著增加训练成本，却未在本任务设置下形成相应收益。来源为 `evaluation/results/repeated_bestof64/repeated_bestof64_summary.json`、`artifacts/training_logs/gat_full_train_20260707_133559_output.log` 和 `artifacts/training_logs/mlp_ranked_train_output.log`。耗时受 CPU 型号、系统负载和 Python/PyTorch 版本影响，应作为本机实测而非硬件无关常数。
...
2. **奖励塑形阶段：** 背景称 relative_heft 改善至 1.325、cloud_0 降至 96.1%。现存 `evaluation/results/summary_reward_shaped.json` 为 1.367762，`artifacts/diagnostic_logs/reward_shaped_action_distribution_output.log` 为 100%。未找到 1.325 或 96.1% 的原始记录，可能是模型或结果文件曾被覆盖；本报告仅保留设计动机，不采用无法溯源的指标。
3. **归一化后的 cloud_0 占比：** 背景称 96.1%，实际 `artifacts/diagnostic_logs/mlp_normalized_action_distribution_output.log` 为 164/180，即 91.111%。
4. **BC 完整数据准确率：** 背景概述为 98.9%。joint BC 强度测试最高为 99.104%、末轮为 98.880%；ranked 专属完整 BC 数据集末轮/最高为 98.967%。本报告按具体实验分别列出。
5. **训练耗时：** 历史 `artifacts/training_logs/residual_train_ppo_mlp_residual.log` 记录 925.23 秒；后续全新克隆同配置日志记录 489.71 秒。两次均完成 200704 PPO 步，差异可能来自系统负载或软件环境。本报告工程效率优先采用后者，并保留前者作为历史记录。
...
- 动作空间诊断：`artifacts/diagnostic_logs/action_space_complexity_check_output.log`
- 归一化诊断：`artifacts/diagnostic_logs/bc_batch_mask_and_normalization_check_output.log`
